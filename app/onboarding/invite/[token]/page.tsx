...
export default async function InvitePage({
  ...
  if (!user) {
    return (
      <Shell>
        <h1 className="text-xl font-bold text-zinc-900">You&apos;ve been invited</h1>
        <p className="mt-2 text-sm text-zinc-500">
          Join <span className="font-medium text-zinc-800">{preview.org_name}</span>{' '}
          as <span className="font-medium text-zinc-800">{roleLabel}</span>.
        </p>
        <p className="mt-1 text-sm text-zinc-500">
          Sign in or create an account with{' '}
          <span className="font-medium text-zinc-800">{preview.email}</span> to
          accept.
        </p>
  ...
  if (user.email?.toLowerCase() !== preview.email.toLowerCase()) {
    return (
      <Shell>
        ...
        <h1 className="mt-3 text-xl font-bold text-zinc-900">Wrong account</h1>
        <p className="mt-2 text-sm text-zinc-500">
          This invitation was sent to{' '}
          <span className="font-medium text-zinc-800">{preview.email}</span>, but
          you&apos;re signed in as{' '}
