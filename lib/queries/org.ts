...
/** The signed-in user's platform-level profile bits (or null if signed out). */
export async function getViewer(): Promise<Viewer | null> {
  ...
  const { data } = await supabase
    .from('profiles')
    .select('id, email, full_name, is_superadmin, deactivated_at')
    .eq('id', user.id)
    .maybeSingle<Viewer>()
...
/** For /admin — redirects non-superadmins away. */
export async function requireSuperadmin(): Promise<Viewer> {
  const viewer = await getViewer()
  if (!viewer) redirect('/login')
  if (viewer.deactivated_at) redirect('/deactivated')
  if (!viewer.is_superadmin) redirect('/dashboard')
  return viewer
}
